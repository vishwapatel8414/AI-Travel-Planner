...
export default function Flights({ sharedData, liveData, isLoading }) {
  ...
  const [depDate, setDepDate] = useState("11/07/2026");
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {flightTabs.map((tab, idx) => (
          <div key={idx} className={`p-3.5 rounded-xl border text-center ${tab.active ? "bg-indigo-50/50 border-indigo-200 text-indigo-600 font-bold" : "bg-white border-slate-100 text-slate-700"}`}>
      ...
      <div className="space-y-3">
        {isLoading ? (
          <div className="bg-white p-12 rounded-2xl text-center space-y-3"><div className="w-8 h-8 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin mx-auto" /><p className="text-xs font-bold text-slate-500 animate-pulse">Fetching live flights to {toCity}...</p></div>
        ) : (
          flightList.map((flight, idx) => (
            <div key={idx} className="bg-white p-4 rounded-xl border border-slate-100/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 hover:shadow-md transition-all">
              ...
              <div className="flex items-center justify-between md:justify-end gap-6"><p className="text-base font-black text-slate-800">{flight.price}</p><button className="bg-[#4f46e5] text-white font-bold text-xs px-6 py-2.5 rounded-xl shadow-sm">Select</button></div>
            </div>
          ))
        )}
      </div>
